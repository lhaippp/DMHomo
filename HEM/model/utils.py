...
def WarpMat(offset, ori_size, patch_size):
    ...
    src_pt = torch.Tensor([[0, 0], [ori_size[0] - 1, 0], [0, ori_size[1] - 1],
                           [ori_size[0] - 1,
                            ori_size[1] - 1]]).float().to(offset.device).view(
                                1, 4, 2).repeat(batch_size, 1, 1)
...
class DLT(nn.Module):
    ...
    def forward(self, src_pt, dst_pt, method='Axb'):
        ...
        """
        xy1 = torch.cat(
            (src_pt, src_pt.new_ones(self.batch_size, self.nums_pt, 1)),
            dim=-1)
        xyu = torch.cat((xy1, xy1.new_zeros(
            (self.batch_size, self.nums_pt, 3))),
                        dim=-1)
        xyd = torch.cat((xy1.new_zeros(
            (self.batch_size, self.nums_pt, 3)), xy1),
                        dim=-1)
        M1 = torch.cat((xyu, xyd), dim=-1).view(self.batch_size, -1, 6)
        M2 = torch.matmul(dst_pt.view(-1, 2, 1),
                          src_pt.view(-1, 1, 2)).view(self.batch_size, -1, 2)
        ...
        if method == 'Ax0':
            ...
            H = torch.cat((torch.matmul(A_inv, B).view(
                -1, 8), src_pt.new_ones((self.batch_size, 1))),
                          1).view(self.batch_size, 3, 3)
            return H


def WarpImages(input_map, H, start, patch_size):

    def bilinear_interpote(batch_img, batch_x, batch_y, patch_size, bs, imgh,
                           imgw, imgc):
        ...
        batch_pa = torch.gather(batch_img,
                                dim=0,
                                index=Ia.unsqueeze(-1).repeat(1, imgc))
        batch_pb = torch.gather(batch_img,
                                dim=0,
                                index=Ib.unsqueeze(-1).repeat(1, imgc))
        batch_pc = torch.gather(batch_img,
                                dim=0,
                                index=Ic.unsqueeze(-1).repeat(1, imgc))
        batch_pd = torch.gather(batch_img,
                                dim=0,
                                index=Id.unsqueeze(-1).repeat(1, imgc))
        ...
        warp_img = (wa * batch_pa + wb * batch_pb + wc * batch_pc +
                    wd * batch_pd)  # * kw_mask
    ...
    new_batch_xy = torch.zeros(
        (bs, patch_size[0] * patch_size[1], 2)).to(device)
    batch_x = torch.arange(patch_size[0]).repeat(patch_size[1]).view(
        1, -1, 1).repeat(bs, 1, 1).to(device)
    batch_y = torch.repeat_interleave(torch.arange(patch_size[1]), repeats=patch_size[0]).view(1, -1, 1). \
        repeat(bs, 1, 1).to(device)

    goal = torch.cat([
        batch_x, batch_y,
        torch.ones_like(batch_x, dtype=torch.float).to(device)
    ...
    small = 1e-7
    smallers = 1e-6 * (1.0 -
                       torch.ge(torch.abs(img_pt[:, :, 2]), small).float())
    ...
    offset_batch_xy = goal.permute(0, 2,
                                   1)[:, :, :2].contiguous() + start.view(
                                       bs, 1, 2).contiguous() + flow
    new_batch_img = bilinear_interpote(
        input_map.permute(0, 2, 3, 1).view(bs, -1, imgc).contiguous(),
        offset_batch_xy[:, :, 0], offset_batch_xy[:, :, 1], patch_size, bs,
        imgh, imgw, imgc)
    new_batch_img = new_batch_img.permute(0, 3, 1, 2).contiguous()
    return new_batch_img, flow.view(bs, patch_size[1], patch_size[0],
                                    2).contiguous()
...
def CropPatchFromFull(patch_size, full_img, start, rescale=False):
    ...
    if rescale:
        new_batch_img = bilinear_interpote(
            full_img.permute(0, 2, 3, 1).view(bs, -1, c).contiguous(), batch_x,
            batch_y, patch_size, bs, h, w, c)
    else:
        batch_x = batch_x.view(-1, 1)
        batch_y = batch_y.view(-1, 1)

        full_img_flatten = full_img.permute(0, 2, 3,
                                            1).reshape(-1, c).contiguous()
        dim1 = h * w
        dim2 = w

        base = torch.arange(0, bs,
                            dtype=torch.int).to(device).view(-1,
                                                             1).contiguous()
        base = base * dim1
        base = base.repeat_interleave(pw * ph, axis=0)
        base_y0 = base + batch_y * dim2
        Ip = base_y0 + batch_x
        new_batch_img = torch.gather(full_img_flatten,
                                     dim=0,
                                     index=Ip.long().repeat(1, c))

    return new_batch_img.reshape(bs, patch_size[1], patch_size[0],
...
def get_grid(batch_size, H, W, start=0):
    ...
    ones = torch.ones_like(xx).to(
        start.device) if type(start) is not int else torch.ones_like(xx)
...
def get_src_p(batch_size, patch_size_h, patch_size_w, divides, axis_t=False):
    ...
    if axis_t:
        ...
        src_p = torch.cat((xx, yy, ones), 1).repeat(batch_size, 1, 1,
                                                    1).float()
    else:
...
def chunk_2D(img, h_num, w_num, h_dim=2, w_dim=3):
    ...
    return torch.cat(img, dim=1).reshape(bs, c, h_num, w_num, h // h_num,
                                         w // w_num)
...
def get_point_pairs(src_p, divide):  # src_p: shape=(bs, 2, h, w)
    ...
    src_p = src_p.permute(0, 1, 3, 4, 2).reshape(bs, divide * divide, 4,
                                                 2).contiguous()
    return src_p
...
def DLT_solve(src_p, off_set):
    ...
    ones = torch.ones(
        N, 4, 1).cuda() if torch.cuda.is_available() else torch.ones(N, 4, 1)
    xy1 = torch.cat((src_ps, ones), axis=2)
    zeros = torch.zeros_like(
        xy1).cuda() if torch.cuda.is_available() else torch.zeros_like(xy1)
...
def get_flow(H_mat_mul,
    ...
    H_mat_pool = H_mat_mul.reshape(batch_size, divide, divide, 3,
                                   3)  # .transpose(2,1)
    H_mat_pool = H_mat_pool.repeat_interleave(
        small_gap_sz[0], axis=1).repeat_interleave(small_gap_sz[1], axis=2)

    if point_use and H_mat_pool.shape[2] != patch_indices.shape[2]:
        H_mat_pool = H_mat_pool.permute(0, 3, 4, 1, 2).contiguous()
        H_mat_pool = F.pad(H_mat_pool,
                           pad=(0, 1, 0, 1, 0, 0),
                           mode="replicate")
        H_mat_pool = H_mat_pool.permute(0, 3, 4, 1, 2).contiguous()

    pred_I2_index_warp = patch_indices.permute(0, 2, 3,
                                               1).unsqueeze(4).contiguous()

    pred_I2_index_warp = torch.matmul(H_mat_pool,
                                      pred_I2_index_warp).squeeze(-1).permute(
                                          0, 3, 1, 2).contiguous()
...
def transformer(I, vgrid, train=True):
    ...
    def _interpolate(im, x, y, out_size, scale_h):
        ...
        base = base * dim1
        base = base.repeat_interleave(out_height * out_width,
                                      axis=0)  # 其实就是单纯标出batch中每个图的下标位置
...
def get_warp_flow(img, flow, start=0):
    ...
    grid_warp = get_grid(batch_size, patch_size_h, patch_size_w,
                         start)[:, :2, :, :].to(flow.device) + flow
