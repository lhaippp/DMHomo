...
def warp(x, flo, padding_mode='zeros'):
    ...
    if float(torch.__version__.split('.')[1]) >= 3:
        output = nn.functional.grid_sample(x.float(), vgrid.float(), align_corners=True, padding_mode=padding_mode)
    else:
        output = nn.functional.grid_sample(x.float(), vgrid.float(), padding_mode=padding_mode)
    return output
